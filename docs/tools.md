...
Search the web via Exa, Perplexity AI, Gemini, Parallel, a self-hosted SearXNG instance, Olostep, or Brave. Returns a synthesized answer with source citations. The tool result's `details` object includes a per-query `trace` (read-only routing audit: which providers were considered, skipped, errored, and which produced the result) so you can see *why* a provider was chosen.
...
| `provider` | `auto` (default), `priority`, `exa`, `perplexity`, `gemini`, `parallel`, `searxng`, `olostep`, or `brave`. `auto` order is Exa → Perplexity → Gemini → Parallel (last resort). `searxng`, `olostep`, and `brave` are opt-in (not in the `auto` order). Use `priority` to honor the `providerPriority` order |
