...
import { getBrightDataApiKey } from "./brightdata.js";

// Bright Data structured "web_data_*" feeds: async snapshot jobs. POST a
...
// extraction if it does not resolve in time. Ports the feed router from
// upstream #124 (mo-root); JSON responses are narrowed with type guards
// (never unchecked casts) and `delay` uses Promise.withResolvers.
...
// Static lookup: snapshot statuses that mean "still building, poll again".
...
export async function fetchBrightDataFeed(
	...
	try {
		...
		// Narrow the trigger response without an unchecked cast.
		const triggerBody: unknown = await triggerRes.json();
		const snapshotId =
			typeof triggerBody === "object" && triggerBody !== null
			&& "snapshot_id" in triggerBody && typeof triggerBody.snapshot_id === "string"
				? triggerBody.snapshot_id
				: undefined;
		if (!snapshotId) {
		...
		while (Date.now() < deadline) {
			...
			const data: unknown = await snapshotRes.json();
			// A ready snapshot is the records array (or a terminal-status object);
			// only a string `status` in a pending state means "keep polling".
			if (
				typeof data === "object" && data !== null && !Array.isArray(data)
				&& "status" in data && typeof data.status === "string"
				&& (data.status.toLowerCase() in PENDING_STATUSES)
			) {
				continue;
			}
