...
// SearXNG: an optional, self-hosted metasearch provider. Needs no third-party
// API key — only a configured base URL (`searxngBaseUrl` config or
// `SEARXNG_BASE_URL` env). Uses the SearXNG JSON API (`/search?format=json`).
// Ports upstream #107 (marnunez) onto our tree: config access routes through
// the centralized `config.ts` (no local config clone/cache), and search types
// are imported from `perplexity.js` (our tree's shared search-types home, per
// exa/parallel/gemini-search convention).
//
// Opt-in by design: NOT in `DEFAULT_AUTO_ORDER`. Reachable via explicit
// `provider: "searxng"` or a `providerPriority` listing. A set-and-forget
// SearXNG instance therefore never silently becomes a first-choice fallback.
 ...
 * so availability gating treats them as "not configured" (SSRF/parse safety).
 */
...
/** Resolve the SearXNG base URL: env (`SEARXNG_BASE_URL`) takes precedence over
 * the resolved config (`searxngBaseUrl`), matching our global precedence. */
function getBaseUrl(): string | null {
	return normalizeBaseUrl(process.env.SEARXNG_BASE_URL) ?? normalizeBaseUrl(loadWebSearchConfig().searxngBaseUrl);
}
...
function requireBaseUrl(): string {
	...
	if (!baseUrl) {
		throw new Error(
			...
			"  1. Set searxngBaseUrl in the web-search config (e.g. { \"searxngBaseUrl\": \"https://search.example.com\" })\n" +
