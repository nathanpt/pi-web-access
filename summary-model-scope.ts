...
// Summary-model scoping helpers (ports upstream 4c8d4b7 / #73).
//
// Curator summaries and query rewriting resolve models through the active
// registry and respect Pi's `enabledModels` when configured. If no enabled
// summary model is available, the curator returns a deterministic no-billing
// summary instead of falling through to unrelated catalog models.
...
function getAgentDir(): string {
	// pi-core's agent dir (settings.json lives here) — distinct from the
	// web-search config dir. Precedence: PI_CODING_AGENT_DIR → ~/.pi/agent.
