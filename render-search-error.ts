 ...
 *
 * WHY THIS EXISTS: the web_search `renderResult` (index.ts) early-returns a
 * SINGLE line on the error/cancel path, before the collapsed/expanded branch.
 * That makes Ctrl+O (app.tools.expand) flip `expanded` with zero visible
 * effect — a dead-end with no diagnostics. It also discards the partial search
 * results gathered before the cancel, so even an expandable view would have
 * nothing to show.
 *
 ...
 * (pi-tui Text/Box). index.ts.renderResult delegates to it and only applies
 * theme colors + creates Text/Box components.
 *
 * Contract for callers:
 *   const plan = buildSearchErrorPlan(details);
 *   if (plan === null) ... // not an error/cancel result; use the normal renderer
 *   // collapsed: [plan.expanded[0], ...plan.collapsed, plan.expandHint].filter(Boolean)
 *   // expanded:  plan.expanded
 *
 * Ports upstream a3fa148 (#99, iRonin).
 */
...
export interface SearchErrorDetails {
	...
	/** Arbitrary extra diagnostic lines (e.g. URLs, response id) for non-cancel
	 * errors like code_search / fetch_content / get_search_content. Shown in the
	 * expanded view. */
	extraLines?: string[];
}
