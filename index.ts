...
function normalizeProviderInput(value: unknown): SearchProvider | undefined {
	...
	if (normalized === "auto" || normalized === "priority" || normalized === "exa" || normalized === "perplexity" || normalized === "gemini" || normalized === "parallel" || normalized === "searxng") {
		return normalized;
	}
	return "auto";
}
	...
	if (initConfig.webSearch?.enabled !== false) pi.registerTool({
		...
		parameters: Type.Object({
			...
			provider: Type.Optional(
				StringEnum(["auto", "priority", "perplexity", "gemini", "exa", "parallel", "searxng"], { description: "Search provider (default: auto). Use 'priority' to honor the configured providerPriority order." }),
			),
