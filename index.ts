...
const RAW_SEARCH_CONCURRENCY = 4;
...
export default function (pi: ExtensionAPI) {
	...
	const searchDescription = curatorAllowed
		? `Search the web using Exa or Gemini (auto), or any opt-in provider (Perplexity, Parallel, Brave, Tavily, OpenAI, SearXNG, Olostep). Returns an AI-synthesized answer with source citations. For comprehensive research, prefer queries (plural) with 2-4 varied angles over a single query — each query gets its own synthesized answer, so varying phrasing and scope gives much broader coverage. When includeContent is true, full page content is fetched in the background. Searches auto-open the interactive browser curator and stream results live; set workflow to "none" to skip curation. Provider auto-selects: Exa (direct API with key, MCP fallback without — zero-config), else Gemini API (needs key), else Gemini Web (needs a supported Chromium-based browser login). Paid-key providers are OPT-IN — a configured key never silently routes or bills: set provider explicitly to "perplexity" (Perplexity API, needs key), "parallel" (Parallel search API, needs key), "brave" (Brave Search API, configured via braveApiKey / BRAVE_API_KEY), "tavily" (Tavily Search API with a native synthesized answer, configured via tavilyApiKey / TAVILY_API_KEY), "openai" (OpenAI Responses API with a built-in web_search tool, configured via openaiApiKey / OPENAI_API_KEY), "searxng" (a self-hosted SearXNG instance, configured via searxngBaseUrl / SEARXNG_BASE_URL), "firecrawl" (a self-hosted Firecrawl instance, configured via firecrawlBaseUrl / FIRECRAWL_BASE_URL; also used as a fetch_content scrape fallback), or "olostep" (Olostep answers API, configured via olostepApiKey / OLOSTEP_API_KEY; also used as a fetch_content scrape fallback), or list any of them in providerPriority and select provider: "priority".`
		: `Search the web using Exa or Gemini (auto), or any opt-in provider (Perplexity, Parallel, Brave, Tavily, OpenAI, SearXNG, Olostep). Returns an AI-synthesized answer with source citations. For comprehensive research, prefer queries (plural) with 2-4 varied angles over a single query — each query gets its own synthesized answer, so varying phrasing and scope gives much broader coverage. When includeContent is true, full page content is fetched in the background. This install returns raw search results directly; the browser curator workflow is disabled by local config. Provider auto-selects: Exa (direct API with key, MCP fallback without — zero-config), else Gemini API (needs key), else Gemini Web (needs a supported Chromium-based browser login). Paid-key providers are OPT-IN — a configured key never silently routes or bills: set provider explicitly to "perplexity" (Perplexity API, needs key), "parallel" (Parallel search API, needs key), "brave" (Brave Search API, configured via braveApiKey / BRAVE_API_KEY), "tavily" (Tavily Search API with a native synthesized answer, configured via tavilyApiKey / TAVILY_API_KEY), "openai" (OpenAI Responses API with a built-in web_search tool, configured via openaiApiKey / OPENAI_API_KEY), "searxng" (a self-hosted SearXNG instance, configured via searxngBaseUrl / SEARXNG_BASE_URL), "firecrawl" (a self-hosted Firecrawl instance, configured via firecrawlBaseUrl / FIRECRAWL_BASE_URL; also used as a fetch_content scrape fallback), or "olostep" (Olostep answers API, configured via olostepApiKey / OLOSTEP_API_KEY; also used as a fetch_content scrape fallback), or list any of them in providerPriority and select provider: "priority".`;
	...
	async function loadSummaryModelChoices(
		...
		if (!defaultSummaryModel && summaryModels.length > 0 && scopeLoaded) {
	...
	if (initConfig.webSearch?.enabled !== false) pi.registerTool({
		...
		parameters: Type.Object({
			...
			provider: Type.Optional(
				StringEnum(["auto", "priority", "perplexity", "gemini", "exa", "parallel", "searxng", "firecrawl", "olostep", "brave", "tavily", "openai"], { description: "Search provider (default: auto). Use 'priority' to honor the configured providerPriority order." }),
			),
		...
		async execute(callId, params, signal, onUpdate, ctx) {
			...
			onUpdate?.({
				content: [{ type: "text", text: `Searching ${queryList.length} ${queryLabel}...` }],
				details: { phase: "search", progress: 0, queryCount: queryList.length, concurrency: Math.min(RAW_SEARCH_CONCURRENCY, queryList.length) },
			});

			const searchOutcomes = await Promise.all(queryList.map((query, index) => rawSearchLimit(async () => {
				onUpdate?.({
					content: [{ type: "text", text: `Searching ${index + 1}/${queryList.length}: "${query}"...` }],
					details: { phase: "search", progress: completedSearches / queryList.length, currentQuery: query, parallel: true },
				});
				...
				try {
					...
					onUpdate?.({
						...
						details: { phase: "search", progress: completedSearches / queryList.length, completed: completedSearches, queryCount: queryList.length },
					});
				}
			})));

			const searchResults = searchOutcomes.map(outcome => outcome.result);
	...
	pi.registerCommand("curator", {
		...
		handler: async (args, ctx) => {
			...
			if (arg === "never") {
				try {
					saveConfig({ allowCurator: false, workflow: "none" });
				} catch (err) {
					const message = err instanceof Error ? err.message : String(err);
				...
				pi.sendMessage({
					...
					content: [{ type: "text", text: "Curator hard-disabled — web_search will always return raw results" }],
					display: "tool",
					details: { workflow: "none", allowCurator: false },
				}, { triggerTurn: false, deliverAs: "followUp" });
				return;
			}
			...
			if (arg === "allow") {
				...
				pi.sendMessage({
					customType: "curator-config",
					content: [{ type: "text", text: "Curator allowed — use /curator on to enable browser review" }],
					display: "tool",
					details: { allowCurator: true },
				}, { triggerTurn: false, deliverAs: "followUp" });
				return;
			}
			...
			if (arg.length === 0) {
				const current = resolveWorkflow(config.workflow, true, allowed);
