...
function normalizeProviderInput(value: unknown): SearchProvider | undefined {
	...
	if (normalized === "auto" || normalized === "priority" || normalized === "exa" || normalized === "perplexity" || normalized === "gemini" || normalized === "parallel" || normalized === "searxng" || normalized === "olostep" || normalized === "brave" || normalized === "tavily") {
		return normalized;
	}
	return "auto";
}
...
export default function (pi: ExtensionAPI) {
	...
	const searchDescription = curatorAllowed
		? `Search the web using Perplexity AI, Exa, Gemini, or Parallel. Returns an AI-synthesized answer with source citations. For comprehensive research, prefer queries (plural) with 2-4 varied angles over a single query — each query gets its own synthesized answer, so varying phrasing and scope gives much broader coverage. When includeContent is true, full page content is fetched in the background. Searches auto-open the interactive browser curator and stream results live; set workflow to "none" to skip curation. Provider auto-selects: Exa (direct API with key, MCP fallback without), else Perplexity (needs key), else Gemini API (needs key), else Gemini Web (needs a supported Chromium-based browser login), else Parallel (needs a Parallel API key). Set provider explicitly to "parallel" (Parallel search API), "searxng" (a self-hosted SearXNG instance, configured via searxngBaseUrl / SEARXNG_BASE_URL), or "olostep" (Olostep answers API, configured via olostepApiKey / OLOSTEP_API_KEY; also used as a fetch_content scrape fallback), or "brave" (Brave Search API, configured via braveApiKey / BRAVE_API_KEY), or "tavily" (Tavily Search API with a native synthesized answer, configured via tavilyApiKey / TAVILY_API_KEY).`
	...
	if (initConfig.webSearch?.enabled !== false) pi.registerTool({
		...
		parameters: Type.Object({
			...
			provider: Type.Optional(
				StringEnum(["auto", "priority", "perplexity", "gemini", "exa", "parallel", "searxng", "olostep", "brave", "tavily"], { description: "Search provider (default: auto). Use 'priority' to honor the configured providerPriority order." }),
			),
