...
// SSRF protection: validate that a remote URL resolves to a public address
// before it is fetched, and re-validate every redirect target manually so a
// public URL that 302s to an internal/metadata address (169.254.169.254) cannot
// bypass the guard. Ports upstream PR #102 (TianZuo555) onto our tree; we had
// previously shipped the lighter inline `validateUrl` (fork PR #2 / upstream #81),
// which checked only the *initial* URL and only *literal* private IPs.
